...
Status snapshot for the `release-candidate` (2026-09-02; refreshed 2026-09-06 after the full-system audit). Every row records the
...
Test evidence: `python -m pytest tests -q` → **257 passed**, 0 failed; `scripts/smoke_all_routes.py` → 2,587 role/route combinations, 0 white pages; `scripts/profile_queries.py` → all hot pages within budget.
Migration head: `b7c2e9d41f05` (single head, chain applies cleanly to empty DB).
...
| Authorization | GREEN | RBAC + need-to-know rewritten for all 13 roles; supervisory read never unlocks finalized records; labelled previews | none | none | test_negative_authz, test_audit_regressions, test_security_fixes |
...
| Migrations | GREEN | Single head `b7c2e9d41f05`; chain applies to empty DB (also in CI) | none | none | scripts/_mig_chain_check.py |
| Backup | YELLOW | `backup.py` PG/SQLite; checksum+retention; SQLite backup/verify/restore round trip executed 2026-09-06 | none | run real `pg_dump` backup on PythonAnywhere | manual (SQLite) + test_production_hardening |
...
| Performance | YELLOW | FK indexes; per-page SQL budget enforced in CI (inbox N+1 removed); no live latency measurement | none | measure on PA | scripts/profile_queries.py |
