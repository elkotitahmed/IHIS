...
def capabilities():
    capabilities_data = [
        ...
        },
        {
            ...
            'color': 'var(--ihis-info)',
            'items': [
                {'feature': 'AI Clinical Copilot', 'desc': 'Patient summary, differential, documentation drafts, safety review, patient communication — one entry point on every page',
                 'roles': 'Physician, Nurse, Pharmacist, Dentist', 'url': '/ai/hub'},
                {'feature': 'Lab Interpretation', 'desc': 'AI-assisted lab result interpretation',
                 'roles': 'Physician, Lab Technician', 'url': '/ai/lab'},
                {'feature': 'Radiology AI', 'desc': 'Critical-finding engine: rules + local classifier → alert → task → acknowledgement → audit',
                 'roles': 'Radiologist, Physician', 'url': '/ai/copilot/radiology'},
                ...
                {'feature': 'ICD-10 lookup & coding', 'desc': 'Smart diagnosis lookup in the EMR; Gemini coding suggestions per patient',
                 'roles': 'Physician, Admin', 'url': '/ai/hub'},
            ],
        },
        {
